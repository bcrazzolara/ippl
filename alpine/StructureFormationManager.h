...
// define functions used in sampling particles
struct CustomDistributionFunctions1 {
  struct CDF{
       KOKKOS_INLINE_FUNCTION double operator()(double x, unsigned int d, const double *params_p) const {
           return x + (params_p[d] / params_p[d * 2 + 1]) * Kokkos::sin(params_p[d * 2 + 1] * x);
       }
  };
...
class StructureFormationManager : public AlpineManager<T, Dim> {
public:
    ...
    void initializeParticles(){
        ...
        using DistR_t = ippl::random::Distribution<double, Dim, 2 * Dim, CustomDistributionFunctions1>;
        double parR[2 * Dim];
    ...
    void initializeParticlesCSV(){
        Inform m("Initialize Particles");

        auto *mesh = &this->fcontainer_m->getMesh();
        auto *FL = &this->fcontainer_m->getFL();
        using DistR_t = ippl::random::Distribution<double, Dim, 1, CustomDistributionFunctions>;
        ...
        */
        ...
        Kokkos::parallel_for(
            "Assign initial rho", rho.getFieldRangePolicy(),
            KOKKOS_LAMBDA(const int i, const int j, const int k) {
                // local to global index conversion
                rhoview(i, j, k) = 1.0;
                //printf("%d %d %d \n", i, j, k); // 32, 32, 32
        ...
        std::vector<std::vector<int>> data; // Assuming integers in the CSV file
        ...
        while (getline(file, line)) {
            ...
            while (getline(ss, cell, ',')) {
                // Convert string to integer and add to row
                int value = stoi(cell);
                row.push_back(value);
            }
    ...
    void dump() override {
        ...
        dumpStructure(this->fcontainer_m->getE().getView());
        IpplTimings::stopTimer(dumpDataTimer);
    }

    template <typename View>
    void dumpStructure(const View& Eview) {
